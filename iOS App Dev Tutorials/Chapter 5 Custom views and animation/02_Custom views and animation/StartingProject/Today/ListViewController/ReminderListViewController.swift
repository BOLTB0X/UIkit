...
class ReminderListViewController: UICollectionViewController {
    ...
    // MARK: - viewWillAppear
    // viewWillAppear(_:)를 재정의하고 refreshBackground()를 호출
    ...
    // MAKR: - refreshBackground
    func refreshBackground() {
        collectionView.backgroundView = nil // collection view의 backgroundView 속성에 nil을 할당
        let backgroundView = UIView() // backgroundView라는 새 UIView 상수
        let gradientLayer = CAGradientLayer.gradientLayer(for: listStyle, in: collectionView.frame) // CAGradientLayer.gradientLayer(for:in:)의 결과를 gradientLayer라는 새 상수에 할당

        backgroundView.layer.addSublayer(gradientLayer) // background view 레이어에 gradientLayer를 하위 레이어로 추가
        collectionView.backgroundView = backgroundView // 컬렉션 뷰의 backgroundView 속성에 backgroundView를 할당
    }
